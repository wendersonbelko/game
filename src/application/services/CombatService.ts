...
import { TICK_RATE, PLAYER_SIZE, INFECTION_RADIUS, HOT_SPEED } from '../../domain/constants';
...
export class CombatService {
  ...
  performRaycast(shooter: IPlayer, tx: number, ty: number, angleOffset = 0, isMachinegun = false): void {
    const px = shooter.x + PLAYER_SIZE / 2;
    const py = shooter.y + PLAYER_SIZE / 2;
    ...
    const maxRange = 600;
    if (len > maxRange) { dx = (dx / len) * maxRange; dy = (dy / len) * maxRange; len = maxRange; }
    ...
    for (let d = 0; d < len; d += step) {
      ...
      // 1. Parede
      if (collidesWithWalls(rx - 1, ry - 1, 2, 2, this.state.walls)) {
        finalEx = rx; finalEy = ry;
        hitType = 'wall';
        break;
      }
      ...
      for (const b of this.state.pushables) {
        if (rx >= b.x && rx <= b.x + b.w && ry >= b.y && ry <= b.y + b.h) {
          ...
          if (!collidesWithWalls(tBX + 1, b.y + 1, b.w - 2, b.h - 2, this.state.walls) &&
              !collidesWithBoxes(tBX + 1, b.y + 1, b.w - 2, b.h - 2, this.state.pushables, b.id)) b.x = tBX;
          if (!collidesWithWalls(b.x + 1, tBY + 1, b.w - 2, b.h - 2, this.state.walls) &&
              !collidesWithBoxes(b.x + 1, tBY + 1, b.w - 2, b.h - 2, this.state.pushables, b.id)) b.y = tBY;
          break outer;
        }
      }
      ...
      for (const [id, p2] of this.state.players) {
        ...
        const cx2 = p2.x + PLAYER_SIZE / 2, cy2 = p2.y + PLAYER_SIZE / 2;
        if (Math.sqrt((rx - cx2) ** 2 + (ry - cy2) ** 2) < PLAYER_SIZE / 2 + 2) {
          finalEx = rx; finalEy = ry;
          hitType = 'player';
          ...
          if (!collidesWithWalls(tPX, p2.y, p2.w, p2.h, this.state.walls) &&
              !collidesWithBoxes(tPX, p2.y, p2.w, p2.h, this.state.pushables, -1)) p2.x = tPX;
          if (!collidesWithWalls(p2.x, tPY, p2.w, p2.h, this.state.walls) &&
              !collidesWithBoxes(p2.x, tPY, p2.w, p2.h, this.state.pushables, -1)) p2.y = tPY;
          ...
          if (p2.isHot && !p2.isStunned && p2.reviveImmunityTimer <= 0) {
            ...
            this.pickupSvc.spawnCoinAt(p2.x + PLAYER_SIZE / 2, p2.y + PLAYER_SIZE / 2);

            if (p2.health <= 0) {
              p2.health = 0;
              p2.isStunned = true;
              p2.stunTimer = 10 * TICK_RATE;
  ...
  checkInfection(): void {
    ...
    for (const hot of hots) {
      for (const runner of runners) {
        ...
        if (Math.sqrt(dx * dx + dy * dy) >= infRad) continue;
        ...
        if (runner.shieldTimer > 0) {
          ...
          if (!collidesWithWalls(tHX, hot.y, hot.w, hot.h, this.state.walls) &&
              !collidesWithBoxes(tHX, hot.y, hot.w, hot.h, this.state.pushables, -1)) hot.x = tHX;
          if (!collidesWithWalls(hot.x, tHY, hot.w, hot.h, this.state.walls) &&
              !collidesWithBoxes(hot.x, tHY, hot.w, hot.h, this.state.pushables, -1)) hot.y = tHY;
