...
import {
  PLAYER_SIZE, RUNNER_SPEED, PLAYER_COLORS, MAX_PLAYERS,
  MIN_PLAYERS_TO_START,
} from '../../domain/constants';
import { GamePhase } from '../../domain/enums/GamePhase';

export class JoinPlayerUseCase {
  constructor(
  ...
  execute(ws: WebSocket, name: string, color: string): IPlayer | null {
    if (this.state.players.size >= MAX_PLAYERS) {
      (ws as unknown as { send: (d: string) => void }).send(
        JSON.stringify({ type: 'error', message: 'O servidor está cheio (máximo 20 jogadores).' }),
      );
      return null;
    }
    ...
    const player: IPlayer = {
      ...
      x: spawn.x, y: spawn.y, w: PLAYER_SIZE, h: PLAYER_SIZE,
    ...
    (ws as unknown as { send: (d: string) => void }).send(JSON.stringify({
      type: 'welcome',
      id: pId,
    ...
    setTimeout(() => {
      if (this.state.players.has(pId)) {
        ...
        if (humanCount === 1) {
          (ws as unknown as { send: (d: string) => void }).send(JSON.stringify({ type: 'offerBots' }));
        }
      }
    }, 500);
  ...
  addBots(count: number, broadcaster: IBroadcaster): void {
    if (isNaN(count) || count < 0 || count > 8) return;
    ...
    if (this.state.players.size < MIN_PLAYERS_TO_START && this.state.gamePhase !== GamePhase.LOBBY) {
      this.sessionSvc.resetGame();
    } else if (this.state.gamePhase === GamePhase.LOBBY && this.state.players.size >= MIN_PLAYERS_TO_START) {
      this.sessionSvc.startWarmup();
    }
  }
}
