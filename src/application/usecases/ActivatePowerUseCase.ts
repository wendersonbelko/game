...
export class ActivatePowerUseCase {
  ...
  execute(player: IPlayer, slot: 'Q' | 'E'): void {
    ...
    if (slot === 'Q') player.slotQ = null;
    else              player.slotE = null;

    this._activate(player, powerKey);
  }

  private _activate(player: IPlayer, key: string): void {
    const T = TICK_RATE;
    ...
    switch (key) {
      ...
      case 'blink': {
        let bdx = 0, bdy = 0;
        if (player.input.up)    bdy -= 1;
        if (player.input.down)  bdy += 1;
        if (player.input.left)  bdx -= 1;
        if (player.input.right) bdx += 1;
        ...
        for (let d = dist; d >= 0; d -= 8) {
          const tx = Math.max(TILE, Math.min(MAP_W - TILE - player.w, player.x + ux * d));
          const ty = Math.max(TILE, Math.min(MAP_H - TILE - player.h, player.y + uy * d));
          if (!collidesWithWalls(tx, ty, player.w, player.h, this.state.walls) &&
              !collidesWithBoxes(tx, ty, player.w, player.h, this.state.pushables, -1)) {
