...
class PhysicsService {
    ...
    tickPlayerTimers() {
        ...
        for (const [, p] of this.state.players) {
            ...
            if (p.trackerTimer > 0)
                p.trackerTimer--;
            if (p.phaseshiftTimer > 0)
                p.phaseshiftTimer--;
            if (p.magnetTimer > 0)
                p.magnetTimer--;
            if (p.repelTimer > 0)
                p.repelTimer--;
            if (p.reviveImmunityTimer > 0)
                p.reviveImmunityTimer--;
    ...
    movePlayers() {
        for (const [, p] of this.state.players) {
            ...
            let speed = p.isHot
                ? constants_1.HOT_SPEED * (1 + (p.upgrades?.hunter_speed ?? 0) * 0.05)
                : constants_1.RUNNER_SPEED * (1 + (p.upgrades?.runner_speed ?? 0) * 0.05);
            // Zonas de velocidade
            ...
            if (!p.isHot) {
                ...
                for (const [, p2] of this.state.players) {
                    ...
                    if (Math.sqrt(edx * edx + edy * edy) <= 200) {
                        p.reloadTimer = Math.max(p.reloadTimer, 2 * constants_1.TICK_RATE);
                        p.ammo = 0;
                        p.holdEnergy = 0;
                    }
                }
            ...
            if (hasPhaseShift || !(0, MathUtils_1.collidesWithWalls)(nx, p.y, p.w, p.h, this.state.walls)) {
                p.x = nx;
            }
            const ny = p.y + dy * speed + fy;
            if (hasPhaseShift || !(0, MathUtils_1.collidesWithWalls)(p.x, ny, p.w, p.h, this.state.walls)) {
                p.y = ny;
            }
            ...
            p.x = (0, MathUtils_1.clamp)(p.x, constants_1.TILE, constants_1.MAP_W - constants_1.TILE - p.w);
            p.y = (0, MathUtils_1.clamp)(p.y, constants_1.TILE, constants_1.MAP_H - constants_1.TILE - p.h);
        }
    }
    _calcForces(p) {
        let fx = 0, fy = 0;
        if (!p.isHot) {
            ...
            for (const [, p2] of this.state.players) {
                if (!p2.isHot || p2.magnetTimer <= 0)
                    continue;
                const mdx = (p2.x + constants_1.PLAYER_SIZE / 2) - (p.x + constants_1.PLAYER_SIZE / 2);
                const mdy = (p2.y + constants_1.PLAYER_SIZE / 2) - (p.y + constants_1.PLAYER_SIZE / 2);
            ...
            for (const [, p2] of this.state.players) {
                if (p2.isHot || p2.repelTimer <= 0)
                    continue;
                const rdx = (p.x + constants_1.PLAYER_SIZE / 2) - (p2.x + constants_1.PLAYER_SIZE / 2);
                const rdy = (p.y + constants_1.PLAYER_SIZE / 2) - (p2.y + constants_1.PLAYER_SIZE / 2);
    ...
    _resolveBoxOverlap(p) {
        for (const box of this.state.pushables) {
            ...
            if (overlapX < overlapY) {
                ...
                if (!(0, MathUtils_1.collidesWithWalls)(newBoxX + 1, box.y + 1, box.w - 2, box.h - 2, this.state.walls) &&
                    !(0, MathUtils_1.collidesWithBoxes)(newBoxX + 1, box.y + 1, box.w - 2, box.h - 2, this.state.pushables, box.id)) {
                    box.x = newBoxX;
                }
                else {
                ...
                if (!(0, MathUtils_1.collidesWithWalls)(box.x + 1, newBoxY + 1, box.w - 2, box.h - 2, this.state.walls) &&
                    !(0, MathUtils_1.collidesWithBoxes)(box.x + 1, newBoxY + 1, box.w - 2, box.h - 2, this.state.pushables, box.id)) {
                    box.y = newBoxY;
                }
                else {
    ...
    moveDraggedBoxes() {
        for (const box of this.state.pushables) {
            ...
            if (dist < 1)
                continue;
            const spd = Math.min(constants_1.DRAG_SPEED, dist);
            const mx = (ddx / dist) * spd, my = (ddy / dist) * spd;
            const nxB = box.x + mx;
            if (!(0, MathUtils_1.collidesWithWalls)(nxB + 1, box.y + 1, box.w - 2, box.h - 2, this.state.walls) &&
                !(0, MathUtils_1.collidesWithBoxes)(nxB + 1, box.y + 1, box.w - 2, box.h - 2, this.state.pushables, box.id))
                box.x = nxB;
            const nyB = box.y + my;
            if (!(0, MathUtils_1.collidesWithWalls)(box.x + 1, nyB + 1, box.w - 2, box.h - 2, this.state.walls) &&
                !(0, MathUtils_1.collidesWithBoxes)(box.x + 1, nyB + 1, box.w - 2, box.h - 2, this.state.pushables, box.id))
                box.y = nyB;
        }
    }
    // ─── Vórtex Magnético sobre Caixas ──────────────────────────────────────
    applyMagneticForceOnBoxes() {
        for (const box of this.state.pushables) {
            let bfx = 0, bfy = 0;
            for (const [, p] of this.state.players) {
                if (!p.isHot || p.magnetTimer <= 0)
                    continue;
                const bdx = (p.x + constants_1.PLAYER_SIZE / 2) - (box.x + box.w / 2);
                const bdy = (p.y + constants_1.PLAYER_SIZE / 2) - (box.y + box.h / 2);
            ...
            if (bfx !== 0 || bfy !== 0) {
                ...
                if (!(0, MathUtils_1.collidesWithWalls)(nxBoxX + 1, box.y + 1, box.w - 2, box.h - 2, this.state.walls) &&
                    !(0, MathUtils_1.collidesWithBoxes)(nxBoxX + 1, box.y + 1, box.w - 2, box.h - 2, this.state.pushables, box.id))
                    box.x = nxBoxX;
                const nxBoxY = box.y + bfy;
                if (!(0, MathUtils_1.collidesWithWalls)(box.x + 1, nxBoxY + 1, box.w - 2, box.h - 2, this.state.walls) &&
                    !(0, MathUtils_1.collidesWithBoxes)(box.x + 1, nxBoxY + 1, box.w - 2, box.h - 2, this.state.pushables, box.id))
                    box.y = nxBoxY;
            }
        }
    }
