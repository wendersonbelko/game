 ...
 */
class BotAIService {
    constructor(state, broadcaster, pickupSvc, combatSvc) {
        this.state = state;
    ...
    updateBotAI(p) {
        ...
        p.input.right = false;
        p.input.shift = false;
        const isFrozen = (this.state.gamePhase !== GamePhase_1.GamePhase.INGAME && this.state.gamePhase !== GamePhase_1.GamePhase.WARMUP) ||
            (this.state.gamePhase === GamePhase_1.GamePhase.INGAME && this.state.introFreezeTimer > 0);
        if (isFrozen) {
            p.mouseWorld = null;
            if (p.grabbedBox) {
        ...
        if (stuck && ai.stuckTicks > 2) {
            if (ai.unstuckTimer <= 0) {
                ai.unstuckTimer = Math.floor(0.75 * constants_1.TICK_RATE);
        ...
        this._tryBotBuy(p);
        if (p.isHot)
            this._hotBotAI(p, env);
        else
            this._runnerBotAI(p, env);
        ...
        if (p.grabbedBox) {
            ...
            p.mouseWorld = {
                x: p.x + constants_1.PLAYER_SIZE / 2 - (mx / len) * 50,
                y: p.y + constants_1.PLAYER_SIZE / 2 - (my / len) * 50,
            };
        }
        else {
            p.mouseWorld = null;
    ...
    _hotBotAI(p, env) {
        const ai = p.ai;
        if (ai.unstuckTimer > 0) {
            this._applyDir(p, ai.unstuckAngle, env);
            p.input.shift = true;
            return;
        }
        const runners = [];
        for (const [, p2] of this.state.players) {
            if (p2.isHot || p2.invisibilityTimer > 0)
                continue;
        ...
        if (runners.length > 1) {
            ...
            for (const r of runners) {
                ...
                if (r.p.stamina < 100)
                    score += 120;
                if (r.p.speedDebuffTimer > 0)
                    score += 180;
                if (r.p.shieldTimer > 0)
                    score -= 200;
                if (r.p.repelTimer > 0)
                    score -= 300;
                if (r.p.phaseshiftTimer > 0)
                    score -= 150;
                if (score > bestScore) {
                    bestScore = score;
                    target = r;
        ...
        if (target.p.input) {
            let px2 = 0, py2 = 0;
            if (target.p.input.up)
                py2 -= 1;
            if (target.p.input.down)
                py2 += 1;
            if (target.p.input.left)
                px2 -= 1;
            if (target.p.input.right)
                px2 += 1;
            const pLen = Math.sqrt(px2 * px2 + py2 * py2) || 1;
            const predFrames = Math.min(20, target.dist / (constants_1.HOT_SPEED * 2));
            tx += (px2 / pLen) * constants_1.RUNNER_SPEED * predFrames * 0.7;
            ty += (py2 / pLen) * constants_1.RUNNER_SPEED * predFrames * 0.7;
        }
        let chaseAngle = Math.atan2(ty - p.y, tx - p.x);
        if (target.dist > 350)
            chaseAngle += ai.strafeDir * 0.2;
        this._applyDir(p, chaseAngle, env);
        if (target.dist < 400 && p.stamina > 60)
            p.input.shift = true;
        else if (target.dist < 200)
            p.input.shift = true;
    ...
    _runnerBotAI(p, env) {
        ...
        // Poderes
        if (nearHot)
            this._tryActivatePowers(p, nearHot, hotDist);
        // Tiro tático
        if (ai.shootCooldown <= 0 && p.ammo > 0 && p.reloadTimer <= 0) {
            if (nearHot && hotDist < 550) {
                ...
                if (Math.random() < chance) {
                    ...
                    this._triggerBotShoot(p, nearHot.x + constants_1.PLAYER_SIZE / 2 + noise, nearHot.y + constants_1.PLAYER_SIZE / 2 + noise);
            ...
            if (danger && p.ammo > 1 && Math.random() < 0.03) {
                for (const [, p2] of this.state.players) {
                    if (p2.id === p.id || p2.isHot || !nearHot)
                        continue;
                    const d = Math.sqrt((p2.x - p.x) ** 2 + (p2.y - p.y) ** 2);
                    if (d < 250 && Math.sqrt((p2.x - nearHot.x) ** 2 + (p2.y - nearHot.y) ** 2) > hotDist) {
                        this._triggerBotShoot(p, p2.x + constants_1.PLAYER_SIZE / 2, p2.y + constants_1.PLAYER_SIZE / 2);
                        ai.shootCooldown = 40;
                        break;
                    }
                }
        ...
        if (safe && p.ammo < 3 && p.reloadTimer <= 0) {
            p.reloadTimer = Math.floor(2.5 * constants_1.TICK_RATE);
            p.ammo = 0;
        }
        // Barricada de caixas
        if (ai.boxCooldown > 0)
        ...
        if (!p.grabbedBox && p.holdEnergy > 150 && ai.boxCooldown <= 0) {
            ...
            if (wantToGrab) {
                ...
                if (closestBox) {
                    ...
                    ai.boxCooldown = 4 * constants_1.TICK_RATE;
                }
            }
        }
        ...
        if (p.grabbedBox) {
            ...
            if (!shouldRelease && box && this._isInNarrowPassage(p.x, p.y, p.w, p.h))
                shouldRelease = true;
            if (shouldRelease) {
                if (box)
                    box.grabbedBy = null;
                p.grabbedBox = null;
                p.mouseWorld = null;
                ai.boxCooldown = 2 * constants_1.TICK_RATE;
            }
        }
        ...
        if (nearHot && hotDist < 400) {
            ...
            if (danger)
                fleeAngle += ai.strafeDir * 0.3;
            this._applyDir(p, fleeAngle, env);
            if (critical || p.stamina > 100)
                p.input.shift = true;
        }
    ...
    // ─── Helpers de IA ────────────────────────────────────────────────────────
    _triggerBotShoot(bot, tx, ty) {
        if (bot.isHot || bot.isStunned || bot.reloadTimer > 0 || bot.ammo <= 0)
            return;
        const dx = tx - (bot.x + constants_1.PLAYER_SIZE / 2), dy = ty - (bot.y + constants_1.PLAYER_SIZE / 2);
        ...
        if (bot.ammo <= 0) {
            ...
            bot.reloadTimer = Math.round(5 * constants_1.TICK_RATE * cdMult);
        }
    ...
    _tryActivatePowers(p, hot, dist) {
        const trySlot = (slot, key) => {
            if (!key)
                return false;
            let should = false;
            if (key === 'blink' && dist < 150)
                should = true;
            if (key === 'phaseshift' && dist < 180)
                should = true;
            if (key === 'shield' && dist < 200)
                should = true;
            if (key === 'repel' && dist < 130)
                should = true;
            if (key === 'speed' && dist < 300 && p.stamina < 200)
                should = true;
            if (key === 'invisibility' && dist < 300)
                should = true;
            if (key === 'machinegun' && dist < 450)
                should = true;
            if (!should)
                return false;
            if (slot === 'Q')
                ...
                p.slotE = null;
            if (key === 'phaseshift') {
                p.phaseshiftTimer = 4 * constants_1.TICK_RATE;
                ...
                if (bl > 0) {
                    ...
                    for (let d = maxDist; d >= 0; d -= 8) {
                        const tx = Math.max(constants_1.TILE, Math.min(constants_1.MAP_W - constants_1.TILE - p.w, p.x + ux * d));
                        const ty = Math.max(constants_1.TILE, Math.min(constants_1.MAP_H - constants_1.TILE - p.h, p.y + uy * d));
                        if (!(0, MathUtils_1.collidesWithWalls)(tx, ty, p.w, p.h, this.state.walls) &&
                            !(0, MathUtils_1.collidesWithBoxes)(tx, ty, p.w, p.h, this.state.pushables, -1)) {
                            p.x = tx;
                            p.y = ty;
                ...
                p.speedBoostTimer = 15 * constants_1.TICK_RATE;
                this.broadcaster.broadcast({ type: 'powerActivated', playerId: p.id, powerType: 'speed' });
            }
            else if (key === 'machinegun') {
                p.machinegunTimer = 15 * constants_1.TICK_RATE;
                this.broadcaster.broadcast({ type: 'powerActivated', playerId: p.id, powerType: 'machinegun' });
            }
            else if (key === 'shield') {
                p.shieldTimer = 15 * constants_1.TICK_RATE;
                this.broadcaster.broadcast({ type: 'powerActivated', playerId: p.id, powerType: 'shield' });
            }
            else if (key === 'invisibility') {
                p.invisibilityTimer = 10 * constants_1.TICK_RATE;
                this.broadcaster.broadcast({ type: 'powerActivated', playerId: p.id, powerType: 'invisibility' });
            }
            else if (key === 'repel') {
                p.repelTimer = 6 * constants_1.TICK_RATE;
                this.broadcaster.broadcast({ type: 'powerActivated', playerId: p.id, powerType: 'repel' });
            }
            return true;
        };
        if (p.slotQ && !trySlot('Q', p.slotQ) && p.slotE)
            trySlot('E', p.slotE);
        else if (!p.slotQ && p.slotE)
            trySlot('E', p.slotE);
    }
    _tryBotBuy(p) {
        const ai = p.ai;
        ...
        if (p.isHot) {
            ...
            if (affordable.length > 0) {
                ...
                if (item.id === 'speed')
                    ...
                    p.trackerTimer = this.pickupSvc._getUpgradedTimer(p, 'tracker', 10 * T);
                else if (item.id === 'gravity')
                    p.gravityTimer = 12 * T;
                else if (item.id === 'magnetic')
                    p.magnetTimer = 8 * T;
                else if (item.id === 'supernova')
                    p.supernovaTimer = 10 * T;
                else if (item.id === 'emp')
                    p.empTimer = this.pickupSvc._getUpgradedTimer(p, 'emp', 10 * T);
                this.broadcaster.broadcast({ type: 'itemBought', playerId: p.id, itemId: item.id, coins: p.coins });
                ai.buyCooldown = 2 * constants_1.TICK_RATE;
            }
        }
        else {
            if (p.slotQ && p.slotE)
            ...
            if (ai.personality === 'aggressive')
                items.unshift({ id: 'machinegun', price: 4 });
            else if (ai.personality === 'defensive')
                items.unshift({ id: 'shield', price: 4 });
            const affordable = items.filter(i => mc >= i.price);
            if (affordable.length > 0) {
    ...
    _findBestCollectable(p, maxRange) {
        let best = null;
        let bestScore = -Infinity;
        const pcx = p.x + constants_1.PLAYER_SIZE / 2, pcy = p.y + constants_1.PLAYER_SIZE / 2;
        ...
        for (const c of this.state.coins) {
            ...
            if (d < maxRange) {
                ...
                if (s > bestScore) {
                    bestScore = s;
                    best = { x: c.x + 8, y: c.y + 8 };
                }
            }
        }
        for (const pk of this.state.pickups) {
            if (p.isHot && !hotTypes.includes(pk.type))
                continue;
            if (!p.isHot && !runTypes.includes(pk.type))
                continue;
            if (!p.isHot && p.slotQ && p.slotE)
                continue;
            ...
            if (d < maxRange) {
                ...
                if (!p.isHot && (!p.slotQ || !p.slotE))
                    v += 80;
                if (!p.isHot && ['shield', 'blink', 'phaseshift'].includes(pk.type))
                    v += 50;
                if (p.isHot && ['speed', 'magnetic', 'gravity'].includes(pk.type))
                    v += 50;
                const s = v - d * 0.15;
                if (s > bestScore) {
                    bestScore = s;
                    best = { x: pk.x + 10, y: pk.y + 10 };
                }
            }
        }
        return best;
    }
    _smartWander(p, env, range) {
        const ai = p.ai;
        ...
        if (Math.sqrt(dx * dx + dy * dy) < 30)
            ...
            this._applyDir(p, Math.atan2(dy, dx), env);
    }
    _checkBotStuck(p) {
        const ai = p.ai;
    ...
    _findSmartDir(p, idealAngle) {
        ...
        for (const testDist of [24, 14, 7]) {
            for (const offset of sweeps) {
                ...
                const wallOk = !(0, MathUtils_1.collidesWithWalls)(tx, ty, p.w, p.h, this.state.walls);
                const boxOk = testDist <= 7 || !(0, MathUtils_1.collidesWithBoxes)(tx, ty, p.w, p.h, this.state.pushables, -1);
    ...
    _calcEnvForces(p) {
        ...
        for (const [, p2] of this.state.players) {
            if (p2.id === p.id || p.isHot !== p2.isHot)
                continue;
    ...
    _isInNarrowPassage(x, y, w, h) {
        const walls = this.state.walls;
        if ((0, MathUtils_1.collidesWithWalls)(x - 50, y, w, h, walls) && (0, MathUtils_1.collidesWithWalls)(x + 50, y, w, h, walls))
            return true;
        if ((0, MathUtils_1.collidesWithWalls)(x, y - 50, w, h, walls) && (0, MathUtils_1.collidesWithWalls)(x, y + 50, w, h, walls))
            return true;
        return false;
    }
    _ensureBotAI(p) {
...
exports.BotAIService = BotAIService;
//# sourceMappingURL=BotAIService.js.map