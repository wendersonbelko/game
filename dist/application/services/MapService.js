...
class MapService {
    ...
    generateMap() {
        ...
        const T = constants_1.TILE;
        const addWall = (x, y, w, h) => {
            this.state.walls.push({ x, y, w, h });
        };
        // ── Borda externa ──
        addWall(0, 0, constants_1.MAP_W, T);
        addWall(0, constants_1.MAP_H - T, constants_1.MAP_W, T);
        addWall(0, 0, T, constants_1.MAP_H);
        addWall(constants_1.MAP_W - T, 0, T, constants_1.MAP_H);
        ...
        while (pillarsAdded < numPillars && pillarAttempts < 100) {
            ...
            if (!(0, MathUtils_1.collidesWithWalls)(px - T, py - T, pw + 2 * T, ph + 2 * T, this.state.walls)) {
                addWall(px, py, pw, ph);
                pillarsAdded++;
            }
        }
        ...
        for (const def of zoneDefs) {
            ...
            while (!placed && attempts < 100) {
                ...
                const collidesW = this.state.walls.some(w => (0, MathUtils_1.rectOverlap)(zx, zy, zw, zh, w.x, w.y, w.w, w.h));
                const collidesZ = this.state.speedZones.some(z => (0, MathUtils_1.rectOverlap)(zx, zy, zw, zh, z.x, z.y, z.w, z.h));
        ...
        for (const size of boxSizesList) {
            ...
            while (!placed && attempts < 150) {
                ...
                if (!(0, MathUtils_1.collidesWithWalls)(rx - 2, ry - 2, s + 4, s + 4, this.state.walls) &&
                    !(0, MathUtils_1.collidesWithBoxes)(rx - 2, ry - 2, s + 4, s + 4, this.state.pushables, -1)) {
    ...
    findSpawnPos() {
        const hx = 3 * constants_1.TILE + constants_1.TILE, hy = 3 * constants_1.TILE + constants_1.TILE;
        const hw = 60 * constants_1.TILE, hh = 42 * constants_1.TILE;
        // Tentativa 1: livre de paredes e caixas
        for (let i = 0; i < 300; i++) {
            const x = hx + Math.random() * (hw - constants_1.PLAYER_SIZE);
            const y = hy + Math.random() * (hh - constants_1.PLAYER_SIZE);
            if (!(0, MathUtils_1.collidesWithWalls)(x, y, constants_1.PLAYER_SIZE, constants_1.PLAYER_SIZE, this.state.walls) &&
                !(0, MathUtils_1.collidesWithBoxes)(x, y, constants_1.PLAYER_SIZE, constants_1.PLAYER_SIZE, this.state.pushables, -1))
                return { x, y };
        }
    ...
    getMapData() {
        return {
            w: constants_1.MAP_W, h: constants_1.MAP_H, tile: constants_1.TILE,
